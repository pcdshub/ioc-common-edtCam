           ...
           int baud, int readonly, int channel)
{
	...
	/* flush any junk */
	nAvailToRead = pdv_serial_get_numbytes( ed );
    if ( nAvailToRead > 0 )
	{
    	if (verbose)
    		printf( "flushing %d bytes from serial buffer!\n", nAvailToRead );

    	while ( nAvailToRead > 0 )
		{
			int		nRead;
    		nRead = pdv_serial_read(ed, buf, SERBUFSIZE);
			if( nAvailToRead >= nRead )
				nAvailToRead -= nRead;
			else
				nAvailToRead  = 0;
		}
    ...
    do
    {
		...
		if ( nAvailToRead > 0 )
		{
			printf( "Warning!  %d more bytes have already arrived in serial buffer since we just flushed it!\n",
					nAvailToRead );
		}
