...
int
edt_unmap_dmamem(EdtDev *edt_p)	/* no locking in here.  Be mmap thread-safe in callers! */
{
...
caddr_t
edt_map_dmamem(EdtDev *edt_p)	/* no locking in here.  Be mmap thread-safe in callers! */
{
    ...
    if (ret == ((caddr_t)-1)) {
        perror("mmap call");
        edt_msg( EDTFATAL, "edt_map_dmamem: mmap failed for size %u, offset %d\n", edt_p->totalsize, pagen );
        return(0) ;
    }
...

{
	...
	/* no locking in edt_setup_ring_buffers.  Be mmap thread-safe in callers! */
static int
                       ...
                       int write_flag, unsigned char *pdata, unsigned char **bufarray)
{
    ...
    if (edt_p->mmap_buffers)
    {
		...
		if ( edt_p->base_buffer == 0 )
		{
			edt_msg_perror( EDTFATAL, "edt_setup_ring_buffers: edt_map_dmamem failed\n" );
			return -1;
		}
        bp = edt_p->base_buffer;
...
edt_mapmem(EdtDev *edt_p, u_int addr, int size)
{
    ...
    if (ret == ((caddr_t)-1)) {
        perror("mmap call");
        edt_msg( EDTFATAL, "edt_map_mem: mmap failed for size %d, offset %u\n", size, addr );
        return(0) ;
    }
    return(ret) ;
#endif
...
*/
    ...
    case PE4DVVLSIM_ID:   return ("VisionLink S4");
    ...
    case PE4BL_RADIO_ID:  return("pcie4 lcr radio blade");
    case PE1BL_TIMING_ID: return("pcie1 lcr timing blade");
    case PE8BL_10GNI_ID:  return("pcie8 lcr 10gni blade");
